...
(async()=>{
  ...
  try {
    for(const width of [1440,390]){
      ...
      assert.equal(stats.total,74347);
      ...
      const messina=registry.records.filter(r=>r.authority_key==='messina');
      assert.equal(messina.length,1034);
      assert.equal(messina.filter(r=>r.source_key==='messina-listed').length,880);
      assert.equal(messina.filter(r=>r.source_key==='messina-applicants').length,154);
      assert.deepEqual(statusCounts(messina),{listed:663,pending:154,renewal_update_in_progress:217});
      assert.equal(new Set(messina.map(r=>r.record_locator)).size,1034);
      assert.equal(messina.filter(r=>r.identifiers.length>0).length,1033);
      assert.equal(messina.filter(r=>!r.identifier_field_raw&&r.identifiers.length===0).length,1);
      ...
      const cuneo=registry.records.filter(r=>r.authority_key==='cuneo');
      ...
      const fermo=registry.records.filter(r=>r.authority_key==='fermo');
      ...
      const grosseto=registry.records.filter(r=>r.authority_key==='grosseto');
      ...
      const imperia=registry.records.filter(r=>r.authority_key==='imperia');
      assert.equal(imperia.length,273);
      assert.equal(imperia.filter(r=>r.source_key==='imperia-listed').length,142);
      assert.equal(imperia.filter(r=>r.source_key==='imperia-applicants').length,131);
      assert.deepEqual(statusCounts(imperia),{cancellation_related:5,listed:222,other_or_unknown:3,pending:6,renewal_update_in_progress:37});
      assert.equal(new Set(imperia.map(r=>r.record_locator)).size,273);
      assert.equal(imperia.filter(r=>r.identifiers.length>0).length,257);
      const lucca=registry.records.filter(r=>r.authority_key==='lucca');
      ...
      const trapani=registry.records.filter(r=>r.authority_key==='trapani');
      ...
      const palermo=registry.records.filter(r=>r.authority_key==='palermo');
      ...
      const siracusa=registry.records.filter(r=>r.authority_key==='siracusa');
      ...
      const pescara=registry.records.filter(r=>r.authority_key==='pescara');
      ...
      const piacenza=registry.records.filter(r=>r.authority_key==='piacenza');
      ...
      const prato=registry.records.filter(r=>r.authority_key==='prato');
      assert.equal(prato.length,160);
      assert.equal(prato.filter(r=>r.source_key==='prato-listed').length,137);
      assert.equal(prato.filter(r=>r.source_key==='prato-applicants').length,23);
      ...
      assert.equal(potenza.filter(r=>r.application_date!=='').length,1036);
      assert.equal(potenza.filter(r=>r.observed_listing_date!=='').length,658);
      assert.equal(potenza.filter(r=>r.observed_expiry_date!=='').length,658);
      ...
      assert.equal(potenza.filter(r=>r.source_fields.physical_locator==='source-id:237').length,0);
      const labella=potenza.filter(r=>r.source_fields.physical_locator==='source-id:399');
      assert.equal(labella.length,1);
      assert.equal(labella[0].name,'LABELLA TRASPORTI S.R.L.');
      assert.equal(labella[0].identifier_field_raw,'01720980760');
      assert.equal(labella[0].source_status,'pending');
      assert.equal(labella[0].application_date,'2026-09-17');
      const solaro=potenza.filter(r=>r.source_fields.physical_locator==='source-id:1584');
      assert.equal(solaro.length,1);
      assert.equal(solaro[0].name,'SOLARO S.R.L.');
      assert.equal(solaro[0].identifier_field_raw,'05974780651');
      assert.equal(solaro[0].source_status,'pending');
      assert.equal(solaro[0].application_date,'2026-09-12');
      const duec=potenza.filter(r=>r.source_fields.physical_locator==='source-id:887');
      assert.equal(duec.length,1);
      assert.equal(duec[0].name,'2 C COSTRUZIONI S.R.L.');
