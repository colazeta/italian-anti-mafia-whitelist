...
def test_every_verified_scope_accounts_for_both_logical_populations():
    report = _report()
    assert report["verified_authority_count"] == 57
    assert report["register_scope_count"] == 59
    assert report["complete_register_scope_count"] == 59
...
def test_recently_resolved_current_pages_cover_both_populations():
    report = _report()
    for authority in ("bari", "udine", "crotone", "cuneo"):
        rows = {
            row["population_target"]: row
            for row in report["rows"]
