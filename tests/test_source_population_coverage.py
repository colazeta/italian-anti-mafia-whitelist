...
def test_combined_series_satisfies_both_targets_without_duplication():
    ...
    enna = [
        row
        for row in report["rows"]
        if row["authority_key"] == "enna"
        and row["regime_code"] == "WL-REGIME-L190-2012"
    ]
    assert {row["population_target"] for row in enna} == {"listed", "applicant"}
    assert {row["coverage_status"] for row in enna} == {"COVERED_COMBINED_SERIES"}
    assert all(row["covering_series_keys"] == ["enna-combined"] for row in enna)
