...
The input envelope is closed and contains only interpretation metadata plus parser output: capture ID, parser name/revision, exact 40-character code revision, configuration hash, processing start/end times, records and diagnostics. It deliberately has no SourceEdition or publication field. Source administrative identity must come from source evidence, not from a parser execution.
...
The CI database regression uses real PostgreSQL constraints and synthetic source rows. It demonstrates idempotence, repeated-capture reuse, parser-revision separation, SourceEdition non-creation, complete records/diagnostics retrieval, immutable snapshots and cross-content rejection. It is not production observation persistence and does not replace the separately required real-provider, national recovery, frozen-release replay or independent restore evidence.
