...
The repository now contains an executable, provider-neutral backup and clean-restore gate for the durable source-evidence archive. **The gate is tooling-ready, not live-verified.** No independent recovery provider, account, bucket or credential is provisioned by this repository, and no such resource is created by the workflow.
...
Raw source bytes remain private throughout this path. They are not committed to Git and are not uploaded as GitHub Actions artifacts. Only JSON recovery receipts are retained as bounded workflow evidence.
...
7. emit a private JSON receipt containing the primary/recovery namespace fingerprints, content identity, recovery location, policy locators and verification time.
