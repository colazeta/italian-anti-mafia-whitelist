...
The activity audit found **11 applicant rows** and **47 listed rows** where the physical source row does not yield a recoverable Roman section prefix. These coordinates are frozen explicitly. The raw activity text is preserved, but `sections` remains empty rather than being reconstructed from similar descriptions elsewhere in the same PDF. This matters because at least one description, `Fornitura di ferro lavorato`, occurs under more than one source section and is therefore not safely inferable from text alone. One of the reviewed applicant rows (`ZANCHI STEFANO`, page 54 row 11) contains a PEC address in the activity band; it is preserved verbatim and not treated as an activity or section. One applicant row (`COBO S.R.L.`, page 11 row 9) and one listed row (`ZANETTI ARTURO & C. S.R.L.`, page 130 row 9) have no recoverable activity value at all and remain blank. Any additional missing section prefix, blank activity, unexpected blank mandatory date, changed row denominator, changed page geometry or new date typography fails closed.
