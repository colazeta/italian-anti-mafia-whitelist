...
A parse snapshot is an interpretation of one immutable ContentObject. The same
parse run may be linked to multiple captures of those unchanged bytes, while a
new parser/configuration/code revision creates a new interpretation. Source
...
def _ensure_capture_link(cur, capture_id: str, parse_run_id) -> bool:
    cur.execute(
        """
...
def persist_parse_snapshot(
    ...
    *,
    ...
    creates a SourceEdition or publishes facts. It requires the capture's
    ContentObject to have already reached durable storage status.
    """
    ...
    return {
        ...
        "capture_link_reused": link_reused,
    }
...
def _load_snapshot_envelope(path: Path) -> dict[str, Any]:
    ...
    if set(payload) != expected:
        raise ValueError(
            "Snapshot input fields must be exactly: " + ", ".join(sorted(expected))
        )
    return payload
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Persist a complete immutable parser observation snapshot for an archived capture."
    )
