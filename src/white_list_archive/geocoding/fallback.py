...
from .pipeline import _insert_candidate, _insert_terminal_result
...
def _expire_ineligible_fallback_results(cur) -> int:
    """Close fallback results whose address no longer satisfies the routing contract."""
    cur.execute(
        """
        UPDATE geo.address_geocode_result g
        SET system_period=tstzrange(lower(g.system_period),CURRENT_TIMESTAMP,'[)')
        WHERE g.routing_stage_code='fallback'
                ...
                AND (
                    ...
                    OR (
                        ...
                        AND EXISTS (
                            ...
                            WHERE ann.address_id=g.address_id
                              AND ann.provider_name='anncsu'
                              AND ann.match_status_code='not_found'
                              AND upper_inf(ann.system_period)
                        )
                        AND NOT EXISTS (
                            SELECT 1 FROM geo.address_geocode_result good
                            WHERE good.address_id=g.address_id
...
def _close_current_fallback_results(cur, address_id: Any) -> None:
    """Make a successful provider refresh authoritative across fallback endpoints."""
    cur.execute(
        """
...
def _select_targets(cur, *, provider_name: str, endpoint: str) -> list[FallbackTarget]:
    cur.execute(
        ...
        JOIN geo.address_country_assessment ca
          ON ca.address_id=a.address_id AND upper_inf(ca.system_period)
        LEFT JOIN LATERAL (
            SELECT g.address_geocode_result_id,g.provider_version
...
def _candidate_country_valid(candidate, country_filter_code: str | None) -> bool:
    if not country_filter_code:
        return True
    return (candidate.country_code or "").upper() == country_filter_code.upper()
...
def _create_run(
    ...
    *,
    ...
    expired_ineligible_count: int,
) -> tuple[Any, Any]:
    now = _utc_now()
    cur.execute(
        """
        INSERT INTO provenance.processing_activity(
            activity_type_code,software_name,software_version,configuration_hash,started_at
    ...
    cur.execute(
        ...
        INSERT INTO geo.fallback_geocode_run(
            ...
            eligible_address_count,selected_address_count,expired_ineligible_count,
            status_code,started_at
        ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,'running',%s)
        RETURNING fallback_geocode_run_id
        """,
        (
            activity_id,provider_name,endpoint,provider_version,provider_data_updated,
            configuration_hash,eligible_address_count,selected_address_count,
            expired_ineligible_count,now,
        ),
    )
...
def _record_item(
    ...
    *,
    ...
    cur.execute(
        ...
        """,
        (
            run_id,target.address_id,target.eligibility_reason_code,target.source_route_code,
            target.source_country_code,target.country_filter_code,
            target.upstream_geocode_result_id,target.upstream_provider_version,
            cache_disposition_code,result_status_code,candidate_count,target.query,
        ),
    )
...
def _finish_run(
    ...
    *,
    ...
    cur.execute(
        ...
        SET reused_current_count=%s,network_query_count=%s,
            candidate_address_count=%s,candidate_row_count=%s,
            not_found_count=%s,error_count=%s,status_code=%s,
            completed_at=%s,error_text=%s
        WHERE fallback_geocode_run_id=%s
        """,
        (
            counts['reused_current_count'],counts['network_query_count'],
            counts['candidate_address_count'],counts['candidate_row_count'],
            counts['not_found_count'],counts['error_count'],status_code,now,error_text,run_id,
        ),
    )
    cur.execute(
...
def fallback_geocode(
    *,
    ...
    """Run managed/self-hosted fallback geocoding on explicitly eligible addresses only."""
    _require_psycopg()
    endpoint = endpoint.strip().rstrip('/')
    client = NominatimClient(
        endpoint,
        allow_public_service=False,
    ...
    with psycopg.connect(dsn,autocommit=True) as conn,conn.cursor() as cur:
        expired_ineligible_count = _expire_ineligible_fallback_results(cur)
        ...
        run_id,activity_id = _create_run(
            ...
            expired_ineligible_count=expired_ineligible_count,
        )
        counts = {
        ...
        try:
            for target in selected:
                ...
                if reusable:
                    ...
                    _record_item(
                        cur,run_id=run_id,target=target,
            ...
            for (query,country_filter),targets in groups.items():
                ...
                try:
                    raw_candidates = client.search(
                        query,limit=search_limit,language=language,countrycodes=country_filter
                    )
                    expected_country = targets[0].country_filter_code
                    candidates = [
                        candidate for candidate in raw_candidates
                        if _candidate_country_valid(candidate,expected_country)
                    ]
                    if raw_candidates and expected_country and not candidates:
                        raise RuntimeError(
                            f'Fallback provider returned no {expected_country} candidate despite country filter'
                        )
                except Exception as exc:
                    payload = {'error_type':type(exc).__name__,'message':str(exc)}
                    for target in targets:
                        _insert_terminal_result(
                            cur,address_id=target.address_id,query=query,match_status='error',
                            provider_name=status.provider_name,endpoint=status.endpoint,
                            provider_version=status.software_version,
                            provider_data_updated=status.data_updated,payload=payload,
                            activity_id=activity_id,routing_stage_code='fallback',
                        ...
                        _record_item(
                            cur,run_id=run_id,target=target,cache_disposition_code='queried',
                            result_status_code='error',candidate_count=0,
                        )
                    continue

                for target in targets:
                    _close_current_fallback_results(cur,target.address_id)
                    if not candidates:
                        _insert_terminal_result(
                            cur,address_id=target.address_id,query=query,match_status='not_found',
                            provider_name=status.provider_name,endpoint=status.endpoint,
                            provider_version=status.software_version,
                            provider_data_updated=status.data_updated,payload={'features':[]},
                            activity_id=activity_id,routing_stage_code='fallback',
                            routing_reason_code=target.eligibility_reason_code,
                            upstream_geocode_result_id=target.upstream_geocode_result_id,
                            fallback_geocode_run_id=run_id,
                        ...
                        _record_item(
                            cur,run_id=run_id,target=target,cache_disposition_code='queried',
                            result_status_code='not_found',candidate_count=0,
                        )
                        continue
                    for candidate in candidates:
                        _insert_candidate(
                            cur,address_id=target.address_id,query=query,candidate=candidate,
                            match_status='candidate',provider_name=status.provider_name,
                            endpoint=status.endpoint,provider_version=status.software_version,
                            provider_data_updated=status.data_updated,activity_id=activity_id,
                    ...
                    _record_item(
                        cur,run_id=run_id,target=target,cache_disposition_code='queried',
                        result_status_code='candidate',candidate_count=len(candidates),
                    )

            if (
                counts['candidate_address_count']+counts['not_found_count']+counts['error_count']
                != len(selected)
            ):
                raise RuntimeError(
                    f'Fallback address accounting does not reconcile: {counts} selected={len(selected)}'
                )
            _finish_run(
                cur,run_id=run_id,activity_id=activity_id,counts=counts,status_code='succeeded'
            )
        except Exception as exc:
            _finish_run(
                cur,run_id=run_id,activity_id=activity_id,counts=counts,
                status_code='failed',error_text=str(exc),
            )
            raise
...
def main() -> None:
    ...
    print(json.dumps(fallback_geocode(
        dsn=args.dsn,endpoint=args.endpoint,min_interval_seconds=args.min_interval_seconds,
        search_limit=args.search_limit,language=args.language,max_addresses=args.max_addresses,
