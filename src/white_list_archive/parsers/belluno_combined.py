...
def _parse_row(row: list[str], cfg: dict[str, Any], row_ordinal: int) -> tuple[dict[str, Any], bool]:
    ...
    if applicant:
        ...
        # One source application date is visibly truncated (07/04/202). It is
        # retained only in source extraction and never repaired canonically.
    ...
    with pdfplumber.open(path) as pdf:
        ...
        for page in data_pages:
            ...
            if page_header_count != 1:
                raise ValueError(f"Expected one exact Belluno header on page {page.page_number}, found {page_header_count}")
